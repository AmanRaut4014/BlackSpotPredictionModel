...
def give_analysis():
    #take_screenshot()
    genai.configure(api_key="AIzaSyDGzHkj0F_ERU3_YkyZFk7vbbSZCkNsjMA")
    # Set up the model
    generation_config = {
        "temperature": 0.9,
        "top_p": 1,
    ...
    image_path = Path("map.jpeg")

    prompt= "Based on the image given about Blackspot analytics in the Karnataka India region. Assume the role of a data analyst. You have to present a police officer with the key observations and useful insights and analytics that will aid in decision-making to improve resource utilization, traffic management, junction control, traffic signal optimization, police bandobast, etc. Also, list all the hotspots (orange marker clusters) and predicted hotspots represented in black circles. "
    image_part = {
...
#     save_path = "map.jpeg"  # Path where the screenshot will be saved
#     time.sleep(4)
...
def update_heatmap(df, year_range, Collision_Type, Severity, month_range, District, Accident_Classification, num_markers, min_samples, show_blackspots, show_marker_cluster, show_heatmap):
    ...
    # Display the map
    st_folium(m, width=1410, height=750)  # Use st_folium instead of folium_static to display Folium map
